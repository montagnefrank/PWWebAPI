...
if (isset($_POST['newprov'])) {
    ...
    $directorio = opendir("../../../assets/img/model/");
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["photoProv"]["tmp_name"], $target_file)) {
            ...
            $msg_logo .= " Foto de modelo cargada exitosamente.";

            $box = "primary";

        } else {

            $msg_logo .= " No se logró actualizar su imagen del vehiculo.";
    ...
    $val_select = "INSERT INTO vehiclemodel(imgModel,titleModel,statusModel) VALUES ('" . $filename . "','" . $_POST['nameProv'] . "','" . $_POST['statusProv'] . "')";
...
if (isset($_POST['editprov'])) {    
    ...
    foreach ($loop as $id){
        if (isset($_POST[$id . '_check'])) {
            $val_select = "UPDATE vehiclemodel SET statusModel ='" . $_POST[$id . '_check'] . "' WHERE idModel = '".$id."'";
            $val_result = $link->query($val_select) or die($link->error);
        } else {
            $val_select = "UPDATE vehiclemodel SET statusModel ='0' WHERE idModel = '".$id."'";
            $val_result = $link->query($val_select) or die($link->error);
        }
    }
    
    $msg_menu = " Modelos Actualizados. ";
    $box = "primary";
    $_SESSION['msg'] = $msg_menu;
    $_SESSION['box'] = $box;
...
if (isset($_POST['deleteprov'])) {
    $val_select = "DELETE FROM vehiclemodel WHERE idModel = '" . $_POST['deleteid'] . "'";
    $val_result = $link->query($val_select) or die($link->error);
    $msg_menu = " Modelo eliminado exitosamente. ";
    $box = "primary";
    $_SESSION['msg'] = $msg_menu;
    $_SESSION['box'] = $box;
}
